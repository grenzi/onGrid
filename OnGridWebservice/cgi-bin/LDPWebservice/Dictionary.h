...
class Dictionary {
	private:
		...
		string output_result(const vector< pair<float,int> >& result, const int& match_num);//output
	...
	public:
		...
		void init(const float &threshold1, const float &threshold2, const bool &case_sensitive);
		...
		string match(string& required_name, int& match_num);
