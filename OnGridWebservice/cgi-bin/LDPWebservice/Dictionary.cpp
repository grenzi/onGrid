...
void Dictionary::init(const float &thd1, const float &thd2, const bool &cs) {
	threshold1 = thd1;
	threshold2 = thd2;
	case_sensitive = cs;
}
...
void Dictionary::exactMatch(string& required_name){
	 vector<string> req_vect;
	 string sub;
	 bool match = true;
	 bool mat = false;
	 string lastCUI;
	...
	for(unsigned int i=0;i<dict.size();i++){
		match = true;
		unsigned int reqSize = req_vect.size();
		vector<string> entryName;
		for(unsigned int j = 0;j<dict.at(i).name.size();j++){
			entryName.push_back(dict.at(i).name.at(j).first);
		}
		if(reqSize==entryName.size()){
			for(unsigned int j=0;j<reqSize;j++){
				if(req_vect.at(j).compare(entryName.at(j))!=0){
					match = false;
					break;
				}
			}
		}else{match = false;}
		if((match==true)&&(dict.at(i).CUI.compare(lastCUI)!=0)){
				lastCUI = dict.at(i).CUI;
				mat = true;
				cout<<"\n"<<dict.at(i).CUI<<"|";
				for(unsigned int x=0;x<entryName.size();x++){
					cout<<entryName.at(x)<<" ";
				}
				cout<<"\n";
		}
...
string Dictionary::output_result(const vector< pair<float,int> >& result, const int& match_num){
...
string Dictionary::match(string& required_name, int& match_num){
	...
	*/
	...
	if(result1.at(0).first>threshold1){
		...
		return output_result(result1,match_num);
	...
	*/
	...
	remove_duplicated_results(result);
	cut_tail(result);
	return output_result(result,match_num);
	for(unsigned int i=0;i<dict.size();i++){
