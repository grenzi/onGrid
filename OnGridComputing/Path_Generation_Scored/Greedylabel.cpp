...
Greedylabel::Greedylabel(Graph& graph, char* in_label, char* out_label, char* in_out_same_label, char* in_out_diff_label, char* special_label):g(graph){
...
void Greedylabel::find_allpath(int src, int trg, vector<vector<int> >& pathcollection, const double& EXPBASE)//find simple kDLS paths. In the end no two paths are the same in pathcollection. 
{
	int pow16=int(pow(2.0,16.0));
	int pow8=int(pow(2.0,8.0));
	...
	{		            
		...
		else
		{
			...
			double score = iter_outgoing->score_out * iter_incoming->score_in / g.get_rank(outgoing_Dest);
			Match temp_match;
			temp_match.pContribution = 1.0/score * (1.0/double(pow(EXPBASE,double(temp_distance-1))));
			...
			stringstream pcon_str, score_str, dist_str, out_off_str, in_off_str, common_str;
			pcon_str << temp_match.pContribution;
			score_str << score;
			dist_str << temp_distance;
			out_off_str << temp_match.out_offset;
			in_off_str << temp_match.in_offset;
			common_str << temp_match.common_dest;
			cout << "all-path Match: pContribution: " << pcon_str.str() << ", score: " << score_str.str() << ", distance: " << dist_str.str() << ", out_offset: " << out_off_str.str() << ", in_offset: " << in_off_str.str() << ", common_dest: " << common_str.str() << endl;
		}	
	}
	
	// sort matches based on pContribution
...
void Greedylabel::find_allshortestpath(int src, int trg, vector<vector<int> >& pathcollection, const double& EXPBASE)
{//Note by YX: I did not call find FindShortestDist to reduce repeated calculation.
	...
	{		            
		...
		else
		{
			...
			if (temp_distance==distance)
			{
				...
				in_off_str << temp_match.in_offset;
				common_str << temp_match.common_dest;
				cout << "shortest-path Match: pContribution: " << pcon_str.str() << ", score: " << score_str.str() << ", distance: " << dist_str.str() << ", out_offset: " << out_off_str.str() << ", in_offset: " << in_off_str.str() << ", common_dest: " << common_str.str() << endl;
			}
			iter_outgoing++;
			iter_incoming++;			
		}	
	...
	// sort matches based on pContribution
	sort(all_shortestmatch.begin(), all_shortestmatch.end());
