...
class Greedylabel {
	public:
		...
		Greedylabel(Graph& graph, char* in_label, char* out_label, char* in_out_same_label, char* in_out_diff_label, char* special_label);
		~Greedylabel();	
		Match FindShortestDist(int src, int trg);
		void find_oneshortestpath(int src,int trg, vector<int>& path);
		void find_allpath(int src, int trg, vector<vector<int> >& pathcollection, const double& EXPBASE);
		int find_allpathnum(int src, int trg);
		void find_allshortestpath(int src, int trg, vector<vector<int> >& pathcollection, const double& EXPBASE);
		//void displaylabels();*/
	...
	private:
		...
		inline void MergeLabelVectorAscend(int& vertexID, bool incoming, vector<Label>& labelVector, vector<Label>& labelSameVector, vector<LabelDiff>& labelDiffVector, vector<Label>& VertexLabelVector)
		{
			...
			vector<Label>::iterator iter_lV=labelVector.begin();
			vector<Label>::iterator iter_lSV=labelSameVector.begin();
			vector<LabelDiff>::iterator iter_lDV=labelDiffVector.begin();
